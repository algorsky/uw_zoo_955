...
greater_petal_length<- iris%>%
  merge(y = iris_size, by = "Species")%>%
  mutate(greater = Petal.Length >= mn.petal.length)%>%
  count(greater, Species)

  
# Q: How many samples where Petal.Length >= mean.petal.length does each species have
#setosa = 26; versicolor = 27; virginica = 25
...
wt_ht <- left_join(wt, ht, by = "level")

#It adds another level which height doesn't have so it adds an NA for height.
...
tailnumber<- nycflights%>%
  count(tailnum)%>%
  arrange(desc(n))
# N711MQ first one with all the information
most_flights<- planes%>%
  filter(tailnum == "N711MQ")
#Manufacturer = GULFSTREAM AEROSPACE
#model = G1159B
#year = 1976
#type of airplane = Fixed wing multi engine\
